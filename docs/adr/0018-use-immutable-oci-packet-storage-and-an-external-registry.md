...
- Publish bundles through semantic-hash-derived staging tags, retain only digest-qualified references, and independently resolve registry descriptor metadata before acceptance.
...
- Worker publication and reuse independently resolve and re-fetch the digest-qualified object, then validate it against the exact expected PacketRef and manifest digests.
