...
19. `.github/workflows/compile.yml` is the bounded direct compile seam authorized by ADR-0029. It compiles one exact revision and owns no schedule, lease, retry, or registry state; ADR-0016 Model B remains the durable orchestration authority. Do not give the seam orchestration responsibilities without a superseding ADR.
20. Publication through the direct seam must satisfy the full ADR-0018 acceptance contract — semantic-hash staging, digest-qualified references only, independent descriptor resolution, exact packet and bundle-manifest identity, and fail-closed rejection of valid-object substitution and mutable-tag references. Package-write authority belongs only to the publishing job.
